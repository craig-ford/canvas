...
async def list_reviews(canvas_id: UUID, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db_session)):
    """List reviews for a canvas"""
    ...

@router.post("/canvases/{canvas_id}/reviews", response_model=dict, status_code=201)
async def create_review(canvas_id: UUID, review_data: ReviewCreateSchema, current_user=Depends(require_role("admin", "gm")), db: AsyncSession = Depends(get_db_session)):
    """Create new review"""
    ...

@router.get("/reviews/{review_id}", response_model=dict)
async def get_review(review_id: UUID, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db_session)):
