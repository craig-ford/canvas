...
- **Blocks**: T-014 (VBU API Routes), T-015 (Canvas API Routes), T-016 (Thesis API Routes), T-017 (ProofPoint API Routes)
...
| 001-auth | backend/canvas/models/user.py | T-001 | `from canvas.models.user import User, UserRole` |

### Within Feature
| Task | File | What This Task Imports |
|------|------|------------------------|
| T-002 | backend/tests/canvas/test_services_contract.py | CanvasService interface |
| T-003 | backend/canvas/models/vbu.py | VBU model |
| T-003 | backend/canvas/models/canvas.py | Canvas, LifecycleLane models |
| T-003 | backend/canvas/models/thesis.py | Thesis model |
| T-003 | backend/canvas/models/proof_point.py | ProofPoint, ProofPointStatus models |
...
## Logic
1. Import required models and dependencies from cross-feature and within-feature tasks
2. Implement VBU operations with authorization filtering for list_vbus method
3. Auto-create canvas with lifecycle_lane='build' when creating VBU
4. Implement canvas operations with nested data loading using selectinload
5. Implement thesis operations with max 5 constraint validation
6. Implement proof point operations with date parsing for target_review_month
7. Use HTTPException for consistent error responses
8. Apply business rules: GM role filtering, thesis count limits, cascade deletes
9. Handle optional fields and data validation with proper null checks

## Verification
- [ ] All 13 CanvasService methods implemented with real logic
- [ ] VBU creation auto-creates canvas with lifecycle_lane='build'
- [ ] Max 5 theses constraint enforced
- [ ] GM role filtering in list_vbus method
- [ ] Proper error handling with HTTPException
- [ ] Date parsing for target_review_month field
- [ ] Cascade deletes handled by database relationships
