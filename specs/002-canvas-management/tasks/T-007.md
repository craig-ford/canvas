...
## Task: VBU API Integration Tests
...
## Scope
- **CREATE**: `backend/tests/canvas/test_vbu_api.py`
...
## Logic
1. Import test client, auth fixtures, and VBU models
2. Test authorization for all VBU endpoints across admin/GM/viewer roles
3. Test VBU CRUD operations with proper HTTP status codes
4. Test GM can only access own VBUs for read/update operations
5. Test admin can perform all operations on any VBU
6. Test viewer has read-only access to all VBUs
7. Test validation errors return 422 with proper error structure
8. Test pagination parameters and response metadata
9. Test 404 responses for non-existent resources

## Verification
- [ ] All HTTP methods tested with proper authorization
- [ ] Role-based access control verified for each endpoint
- [ ] Validation errors return proper error codes and messages
- [ ] Response envelopes match API specification
