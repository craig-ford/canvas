...
## Task: Thesis API Integration Tests
...
- **Blocks**: T-016 (Thesis API Routes)
- **Requirements**: FR-003
...
## Scope
- **CREATE**: `backend/tests/canvas/test_thesis_api.py`
...
## Logic
1. Import test client, auth fixtures, and Thesis models
2. Test authorization for thesis CRUD endpoints across all roles
3. Test GM can only access theses from own canvases
4. Test admin has full access to all thesis operations
5. Test viewer has read-only access to all theses
6. Test thesis creation with text and order validation
7. Test thesis text updates and cascade deletes
8. Test thesis reordering with order constraint validation
9. Test duplicate order conflicts return 409 status
10. Test 404 responses for non-existent theses

## Verification
- [ ] Authorization tested for all roles and ownership scenarios
- [ ] CRUD operations tested with proper HTTP status codes
- [ ] Thesis ordering constraints validated
- [ ] Reordering endpoint tested with valid and invalid data
