...
class TestThesisAPIValidation:
    ...
    async def test_create_thesis_duplicate_order(self, client: AsyncClient, admin_token: str, canvas_with_thesis: Canvas):
        ...
        assert response.status_code == 409  # Service rejects duplicate orders
...
class TestThesisAPIReordering:
    async def test_reorder_theses_success(self, client: AsyncClient, admin_token: str, canvas_with_theses: Canvas):
        ...
        assert len(data) == 3  # Returns all theses for canvas, not just reordered ones
    ...
    async def test_reorder_theses_invalid_order(self, client: AsyncClient, admin_token: str, canvas_with_thesis: Canvas):
        ...
        assert response.status_code == 422  # DB CHECK constraint rejects order > 5
