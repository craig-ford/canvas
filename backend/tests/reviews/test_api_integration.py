...
class TestReviewAPIIntegration:
    async def test_list_reviews_admin_access_all_canvases(self, client: AsyncClient, admin_token, canvas_a, canvas_b):
        ...
        resp = await client.get(f"/api/canvases/{canvas_a.id}/reviews", headers={"Authorization": f"Bearer {admin_token}"})
    ...
    async def test_list_reviews_viewer_access_all_readonly(self, client: AsyncClient, viewer_token, any_canvas):
        ...
        resp = await client.get(f"/api/canvases/{any_canvas.id}/reviews", headers={"Authorization": f"Bearer {viewer_token}"})
        assert resp.status_code == status.HTTP_200_OK
    ...
    async def test_create_review_admin_any_canvas(self, client: AsyncClient, admin_token, any_canvas):
        ...
        payload = {
            ...
            "currently_testing_id": str(uuid4()),
            "commitments": [{"text": "Test", "order": 1}]
        }
        resp = await client.post(f"/api/canvases/{any_canvas.id}/reviews", json=payload, headers={"Authorization": f"Bearer {admin_token}"})
    ...
    async def test_create_review_gm_own_canvas_only(self, client: AsyncClient, gm_token, own_canvas, other_canvas):
        ...
        resp_other = await client.post(f"/api/canvases/{other_canvas.id}/reviews", json=payload, headers={"Authorization": f"Bearer {gm_token}"})
        assert resp_other.status_code == status.HTTP_403_FORBIDDEN
    ...
    async def test_create_review_viewer_forbidden(self, client: AsyncClient, viewer_token, any_canvas):
        ...
        resp = await client.post(f"/api/canvases/{any_canvas.id}/reviews", json=payload, headers={"Authorization": f"Bearer {viewer_token}"})
    ...
    async def test_unauthorized_access_returns_401(self, client: AsyncClient, any_canvas):
        ...
        resp = await client.get(f"/api/canvases/{any_canvas.id}/reviews")
