...
detect_progress() {
    ...
    # Method 1: Claude reported file changes
    [[ $files_changed -gt 0 ]] && { echo "files_changed"; return; }
    
    # Method 2: New git commits since last check
    local last_commit_file="$project_dir/logs/.last_commit_hash"
    local current_commit=$(git -C "$project_dir" rev-parse HEAD 2>/dev/null)
    if [[ -n "$current_commit" ]]; then
        local last_commit=$(cat "$last_commit_file" 2>/dev/null)
        if [[ "$current_commit" != "$last_commit" ]]; then
            mkdir -p "$project_dir/logs"
            echo "$current_commit" > "$last_commit_file"
            echo "git_commit"; return
        fi
    fi
    
    # Method 3: Uncommitted changes in working tree
    git -C "$project_dir" status --porcelain 2>/dev/null | grep -q . && { echo "working_tree"; return; }
    
    # Method 4: Spec files modified recently (within last 2 minutes)
    find "$project_dir/specs" -name "*.md" -mmin -2 2>/dev/null | grep -q . && { echo "spec_files"; return; }
...
record_loop_result() {
    ...
    # Detect progress from multiple sources
    local progress_source=$(detect_progress "$files_changed" "$(pwd)")
    local has_progress=false
    if [[ "$progress_source" != "none" ]]; then
        has_progress=true
        consecutive_no_progress=0
...
should_halt_execution() {
    ...
    if [[ "$state" == "$CB_STATE_OPEN" ]]; then
        ...
        echo "  • Project may be complete (check .kiro/fix_plan.md)"
        echo "  • Claude may be stuck on an error"
        echo "  • .kiro/PROMPT.md may need clarification"
        ...
        echo "  3. Update .kiro/fix_plan.md if needed"
