...
# ONLY detect explicit exit signal from status block - not vague keywords
# CRITICAL: Do NOT match partial phrases like "T-005 is complete" or "phase complete"
COMPLETION_KEYWORDS=("EXIT_SIGNAL: true")
TEST_ONLY_PATTERNS=("npm test" "bats" "pytest" "jest" "cargo test" "go test" "running tests")
# REMOVED: Vague patterns that caused false positives
# These patterns matched mid-sentence phrases like "T-005 is complete" incorrectly
# NO_WORK_PATTERNS - DISABLED, rely only on EXIT_SIGNAL: true
...
analyze_response() {
    ...
    # 5. Detect "nothing to do" patterns - DISABLED
    # These vague patterns caused false positives (e.g., "T-005 is complete" matched "complete")
    # Now we ONLY rely on explicit EXIT_SIGNAL: true in the status block
    # for pattern in "${NO_WORK_PATTERNS[@]}"; do ... done
    ...
    if [[ -f ".last_output_length" && $output_length -gt 0 ]]; then
    ...
    # Only save length if we have actual output
